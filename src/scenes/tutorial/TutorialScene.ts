...
import GameInputManager, {
  KeyData,
} from "../../infrastructure/GameInputManager";
...
import FirstPersonCharacter, {
  CharacterDirection,
} from "../../players/FirstPersonCharacter";
...
export default class TutorialScene implements IGameScene {
  ...
  constructor(private _engine: Engine, private inputManager: GameInputManager) {
    this._onStateChangeObservable.add((s: any) =>
      console.log(`${s.currentState} is new state. Prev was ${s.priorState}`)
    );
  ...
  set gameState(value: number) {
    if (this._state !== value) {
      ...
      this._state = value;
      this._onStateChangeObservable.notifyObservers({
        priorState: this._previousState,
        currentState: value,
      });
    }
  }
  ...
  private async setupTestEnv() {
    ...
    groundMaterial.diffuseTexture = new Texture(
      "/textures/tutorial4texture.jpg",
      this._scene
    );

    const groundMesh = MeshBuilder.CreateGroundFromHeightMap(
      "groundMesh",
      `/textures/tutorial4terrain.png`,
      {
        height: 500,
        width: 500,
        maxHeight: 100,
  ...
  private updateUI() {
    if (this._localPlayer) {
      this._positionBox.text = `X: ${this._localPlayer.PlayerMesh.position.x.toFixed(
        2
      )} - Y: ${this._localPlayer.PlayerMesh.position.y.toFixed(
        2
      )} - Z: ${this._localPlayer?.PlayerMesh.position.z.toFixed(2)}`;
    }
  }

  public async loadPlayer() {
    this._localPlayer = new FirstPersonCharacter(
      this._scene,
      this._camera,
      new Vector3(150, 55, -150),
      { maxPosX: 245, maxPosZ: 245 }
    );
    this.loadPistol();
  }
  ...
  private async loadPistol() {
    const weaponImportResult = await SceneLoader.ImportMeshAsync(
      "",
      "https://raw.githubusercontent.com/joewashek/babylon-assets/main/weapons/",
      "hk_pistol.glb",
      this._scene,
      undefined,
      ".glb"
    ...
    const cubeTarget = weaponImportResult.meshes.find(
      (m) => m.name === "Cube.001_Texture.002_0"
    );
    if (cubeTarget) cubeTarget.isVisible = false;
    const enemyTarget = weaponImportResult.meshes.find(
      (m) => m.name === "enemy_model_Texture.002_0"
    );
  ...
  private MOVE_FORWARD(state: any, data: KeyData) {
    if (data) {
      ...
      this._localPlayer.moveDirection(
