...
import grassWalkingSoundUrl from "../assets/sounds/sfx/03_Step_grass_03.wav";
...
export class ThirdPersonController {
  ...
  private walkingSoundLoaded: boolean = false;
    ...
    this.scene.onBeforeAnimationsObservable.add(() => {
      ...
      if (this.curAnimParam.weight < 1) {
        ...
        this.meshContent.animationGroups.forEach((ani, key) => {
          if (
            key !== this.oldAnimParam.anim &&
            key !== this.curAnimParam.anim
          ) {
  ...
  private onMove() {
    ...
    if (
      ...
      this.player.position.y >
        this.jumpState.startHeight + this.jumpState.limit &&
      this.jumpState.jump
    ) {
      // 落下来
      this.jumpState.jump = false;
    ...
    if (this.iswsad && !this.jumpState.jump) {
      ...
      if (this.wallkingSound.isReady() && !this.wallkingSound.isPlaying) {
        this.wallkingSound.play();
      }
    } else {
      this.velocity.x = 0;
  ...
  private updateState = (): void => {
    ...
    const s_start = this.staircaseRay.origin.add(
      new BABYLON.Vector3(0, 0, 0.03)
    );
    const s_end = s_start.add(new BABYLON.Vector3(0, -0.68, 0));
    (this.physEngine as any).raycastToRef(
      s_start,
      s_end,
      this.staircaseRaycast
    );
